...
async function sendWhatsAppButtons(phoneNumberId, to, options) {
  try {
    const token = process.env.WA_ACCESS_TOKEN;
    if (!token) {
      console.warn('WA_ACCESS_TOKEN is not set; cannot send message.');
    ...
    const status = err.response ? err.response.status : 'unknown';
    const data = err.response ? err.response.data : err.message;
    console.error('Failed to send WhatsApp buttons:', status, data);
  }
}

module.exports = { sendWhatsAppText, sendWhatsAppButtons };
